...
閉じても、次に開いたときに「続きから再開」できます。保存先はブラウザ内のみです。

### キーボードショートカット

| キー | 動作 |
| --- | --- |
| `V` `T` `H` `P` `R` `O` `A` `I` `E` | ツール切り替え |
...
| **PDF注釈** | Square / Circle / Line / Ink / Highlight / FreeText / Stamp 注釈として書き出す。Acrobat などで**あとから選択・移動・削除できる** |
...
- スキャンされた（文字情報を持たない）PDF では、文字の選択・検索・書き換えは
  できません（OCR は未対応）
- パスワード保護された PDF は、書き出すとページが画像になります
- ページを並べ替え・複製・削除・結合すると、しおりやフォームは引き継がれません
  （並びを変えていない単一ファイルなら元の文書構造をそのまま保ちます）
- 斜体は疑似斜体（字送りを傾ける方式）です。日本語フォントに斜体字形が無いためです
- 折り返しの禁則処理は「追い出し」のみで、文字詰め（アキ調整）は行いません
